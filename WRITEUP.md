...
On real dysarthric speech (TORGO) we measure **no gain: 0.196 before, 0.196 after 100 corrections** — and we report it, because the reason is interesting. TORGO's prompts are phonetically-balanced TIMIT sentences, so the vocabulary harvested is ordinary English the recogniser already handles. A previously-corrected word is mis-recognised in only **3 of 25** held-out utterances, capping any possible gain below one WER point — under the noise floor. What TORGO *does* establish is a safety property worth having: **the system never corrupts a transcription it cannot improve.**
